...
from app.services.document_processing import mock_generate_embedding, FAISS_INDEX_PATH, EMBEDDING_DIM
...
@router.post("/ask")
def ask_question(req: AskQuery, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not os.path.exists(FAISS_INDEX_PATH):
        raise HTTPException(status_code=500, detail="Ínidice FAISS no inicializado")
    ...
    # 3. Recuperar fragmentos desde la DB
    resultados = []
